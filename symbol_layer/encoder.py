...
2. 将符号值映射为颜色对 + 图形
3. 渲染到网格的数据模块位置
4. 填充结构区域（Finder、Separator、Format Info）
...
class SymbolEncoder:
    ...
    def _fill_finder_and_separator(self, modules: List[List[RenderedModule]]) -> None:
        """填充标准 Finder Pattern、Separator 和 L 型角标。"""
        N = self.grid.N
        # 标准 Finder (3 个角: TL, TR, BL)
        ...
        # Separator: 白色边框（仅标准 Finder 周围, 3 组）
        for fc, fr in finder_positions:
            for dr in range(-1, 8):
                for dc in range(-1, 8):
                    c, r = fc + dc, fr + dr
        ...
        # L 型角标 (BR, 5 个黑色模块)
        for (c, r) in self.grid._l_corner_coords:
            modules[r][c] = RenderedModule(
                col=c, row=r,
                category=ModuleCategory.AUXILIARY,
                color_a=0, color_b=0,  # 黑
                shape_index=None,
            )
